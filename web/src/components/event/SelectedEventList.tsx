...
export default function SelectedEventList({
  data,
  groupedChoices,
  eventChoicesConfig,
  addEventList,
  ...
  const filtered = useMemo(() => {
    ...
    return selectedEvents?.filter(
      (ev) =>
        ...
        ev.character_name.toLowerCase().includes(val)
    );
          ...
          <div className="border-r p-4 flex flex-col gap-4">
            ...
            <div className="overflow-y-auto px-2">
              ...
              <div className="grid grid-cols-3 gap-3 p-1">
                {allData
                  ?.filter((item) =>
                    characterList.includes(item.name.toLowerCase())
                  )
                  .map((item) => (
                    <button
                      key={item.name}
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {filtered?.length > 0 ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-center text-muted-foreground">
                ...
                <p className="text-sm">
                  Select some from the list to see them here
                </p>
              </div>
            )}
          </div>
