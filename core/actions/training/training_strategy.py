# core/actions/training/training_strategy.py
...
class TrainingStrategy:
    ...
    def select_training(self, results, current_stats, energy_level, year):
        if not results:
            return None

        filtered_data = self._filter_by_stat_caps(results, current_stats)
    ...
    def _filter_by_stat_caps(self, results, current_stats):
        return {
            ...
            if current_stats.get(stat, 0) < config.STAT_CAPS.get(stat, 1200)
        }

    def _training_score(self, x):
        priority_weight = self.PRIORITY_WEIGHTS_LIST[config.PRIORITY_WEIGHT]
        base = x[1]["total_supports"]
        if x[1]["total_hints"] > 0:
            base += 0.5
        multiplier = (
            1
            + config.PRIORITY_EFFECTS_LIST[self.get_stat_priority(x[0])]
            * priority_weight
        )
        ...
        # Debug output
        debug(
            f"{x[0]} -> base={base}, multiplier={multiplier}, total={total}, priority={self.get_stat_priority(x[0])}"
        )

        return (total, -self.get_stat_priority(x[0]))

    # Get priority stat from config
    @staticmethod
    def get_stat_priority(stat_key: str) -> int:
        return (
            config.PRIORITY_STAT.index(stat_key)
    ...
    def _most_support_card(self, results, energy_level):
        # Seperate wit
        ...
        non_wit_results = {
            ...
            if k != "wit" and int(v["failure"]) <= config.MAX_FAILURE
        }

        # Check if train is bad
        all_others_bad = len(non_wit_results) == 0
        if energy_level < config.SKIP_TRAINING_ENERGY:
            info(
        ...
        if (
            all_others_bad
            and wit_data
            and int(wit_data["failure"]) <= config.MAX_FAILURE
            and wit_data["total_supports"] >= 2
        ):
            info(
                "All trainings are unsafe, but WIT is safe and has enough support cards."
            )
            return "wit"

        filtered_results = {
            k: v for k, v in results.items() if int(v["failure"]) <= config.MAX_FAILURE
        }
        ...
        # Best training
        best_training = max(filtered_results.items(), key=self._training_score)
            ...
            if int(best_data["failure"]) == 0:
                ...
                if best_key == "wit":
                    if energy_level > config.NEVER_REST_ENERGY:
                        ...
                        return None
                info(
                    f"Only 1 support but 0% failure. Prioritizing based on priority list: {best_key.upper()}"
                )
                return best_key
            else:
    ...
    # Do rainbow training
    def _rainbow_training(self, results):
        priority_weight = self.PRIORITY_WEIGHTS_LIST[config.PRIORITY_WEIGHT]
        ...
        for stat_name in rainbow_candidates:
            multiplier = (
                1
                + config.PRIORITY_EFFECTS_LIST[self.get_stat_priority(stat_name)]
                * priority_weight
            )
        ...
        best_rainbow = max(
            ...
            key=lambda x: (x[1]["rainbow_points"], -self.get_stat_priority(x[0])),
        )
